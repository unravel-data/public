...
RPMFILE=`curl -sS -u unravel:WillowRoad68 https://preview.unraveldata.com/unravel/staging-RPM/4.6.0/  |grep EMR |awk '{ print $6}' |cut -d\" -f2  |grep 4.6.0.0 |tail -1`
echo $RPMFILE > /tmp/rpmfilename
curl -v -u  unravel:WillowRoad68 https://preview.unraveldata.com/unravel/staging-RPM/4.6.0/${RPMFILE} -o ${RPMFILE}
