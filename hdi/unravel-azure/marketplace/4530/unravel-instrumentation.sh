...
UNRAVEL_PROP='/usr/loca/unravel/etc/unravel.properties'
CLUSTER_TYPE='spark'
...
function getProp() {
  PROP_NAME=$1
  CONF_NAME=$2
  /var/lib/ambari-server/resources/scripts/configs.py -a get -n $CLUSTER_NAME -u $AMBARI_USER -p "$AMBARI_PASS" -c $CONF_NAME --host=headnodehost -f /tmp/unravel/$CONF_NAME
  PROP_VAL=$(python -c "import sys, json; print(json.load(open('/tmp/unravel/$CONF_NAME'))['properties']['$PROP_NAME'])")
  rm "/tmp/unravel/$CONF_NAME"
}
