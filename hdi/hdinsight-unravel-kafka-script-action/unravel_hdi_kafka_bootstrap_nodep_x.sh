...
#  local primary_head_node=$(get_primary_headnode)
  ...
  export KAFKABROKERS=`curl -sS -u $AMBARI_USR:$AMBARI_PWD -G https://${AMBARI_HOST}:8080/api/v1/clusters/$CLUSTER_ID/services/KAFKA/components/KAFKA_BROKER | jq -r '["\(.host_components[].HostRoles.host_name):9092"] | join(",")' | cut -d',' -f1,2`
  export bootstrap_server1=`curl -sS -u $AMBARI_USR:$AMBARI_PWD -G https://${AMBARI_HOST}:8080/api/v1/clusters/$CLUSTER_ID/services/KAFKA/components/KAFKA_BROKER | jq -r '["\(.host_components[].HostRoles.host_name):9092"] | join(",")' | cut -d',' -f1,2|cut -d',' -f1|cut -d'.' -f1`
  export bootstrap_server2=`curl -sS -u $AMBARI_USR:$AMBARI_PWD -G https://${AMBARI_HOST}:8080/api/v1/clusters/$CLUSTER_ID/services/KAFKA/components/KAFKA_BROKER | jq -r '["\(.host_components[].HostRoles.host_name):9092"] | join(",")' | cut -d',' -f1,2|cut -d',' -f2|cut -d'.' -f1`
  export port=`curl -sS -u $AMBARI_USR:$AMBARI_PWD -G https://${AMBARI_HOST}:8080/api/v1/clusters/$CLUSTER_ID/services/KAFKA/components/KAFKA_BROKER | jq -r '["\(.host_components[].HostRoles.host_name):9092"] | join(",")' | cut -d',' -f1,2|cut -d',' -f2|cut -d'.' -f6|cut -d':' -f2`
